...
class TaborProgram:
    def __init__(self,
        ...
        if len(channels) != device_properties['chan_per_part']:
            raise TaborException(
                'TaborProgram only supports {} channels'.format(device_properties['chan_per_part']))
    ...
    def setup_advanced_sequence_mode(self) -> None:
        ...
        for sequencer_table_loop in self.program:
            ...
            for waveform, repetition_count in (
                    (waveform_loop.waveform.get_subset_for_channels(self.__used_channels),
                     waveform_loop.repetition_count)
...
def with_configuration_guard(function_object: Callable[['AWGChannelTupleTabor', Any], Any]) -> Callable[
    ['AWGChannelTupleTabor'], Any]:
    ...
    def guarding_method(channel_pair: 'AWGChannelTupleTabor', *args, **kwargs) -> Any:
...
def with_select(function_object: Callable[['AWGChannelTupleTabor', Any], Any]) -> Callable[['AWGChannelTupleTabor'], Any]:
    """Asserts the channel pair is selcted when the wrapped function is called"""

    @functools.wraps(function_object)
    def selector(channel_pair: 'AWGChannelTupleTabor', *args, **kwargs) -> Any:
        channel_pair.select()
        return function_object(channel_pair, *args, **kwargs)
...
class AWGDeviceTabor(BaseAWG):
    def __init__(self, device_name: str, instr_addr=None, paranoia_level=1, external_trigger=False, reset=False,
        ...
        # TODO: Mirrorfeature ueberarbeiten
        self.add_feature(DeviceMirrorFeature(self.main_instrument, self.mirrored_instruments, self.all_devices))
        ...
        self._channel_tuples.append(AWGChannelTupleTabor(1, self, self._channels[0:1]))
        self._channel_tuples.append(AWGChannelTupleTabor(2, self, self._channels[2:3]))
    ...
    # TODO: Paranoia als Feature?

    # TODO: MirrorFeature
    @paranoia_level.setter
    def paranoia_level(self, val):
        for instr in self[DeviceMirrorFeature].all_devices:
    ...
    def send_query(self, query_str, query_mirrors=False) -> Any:
        if query_mirrors:
            return tuple(instr.send_query(query_str) for instr in self[DeviceMirrorFeature].all_devices)
        else:
    ...
    # def get_amplitude(self, channel) -> float:
    #     if channel not in (1, 2, 3, 4):
    #         raise TaborException('Invalid channel: {}'.format(channel))
    ...
    # TODO: Oder direkt in ein cleanup des Geraets?
    # TODO: Wieso funktionier clear() fuer ChannelTuple nicht
    def reset(self) -> None:
        self.send_cmd(':RES')
        self.initialize()
...
class AWGChannelTupleTabor(BaseAWGChannelTuple):
    def __init__(self, idn: int, tabor_device: AWGDeviceTabor, channels: Iterable["AWGChannelTabor"]):
        ...
        self._channels = tuple(channels)
        # TODO: Standart Sample Rate?

        # Old
        ###########
        # TODO: Braucht man die alte Vererbung?
        ...
        # TODO: Fehler der jetzt beim synchronisiern geworfen wird
    ...
    # New
    ####################################################################################################################
    # TODO: Samplerate ist Channel oder Channeltuple aufgabe?
    # @property
    ...
    def upload(self, name: str,
        ...
        sample_rate = self.device._channels[0].sample_rate()
        ...
        try:
            ...
            #TODO: Iterable does not implements __get_item__
            # They call the peak to peak range amplitude

            ranges = (self.device.channels[0][ChannelAmplitudeOffsetFeature].getAmplitude(),
                        self.device.channels[1][ChannelAmplitudeOffsetFeature].getAmplitude())
            ...
            if self._amplitude_offset_handling == AWGAmplitudeOffsetHandling.IGNORE_OFFSET:
                ...
                voltage_offsets = (self.device._channels[0][ChannelAmplitudeOffsetFeature].get_offset(),
                                   self.device._channels[1][ChannelAmplitudeOffsetFeature].get_offset())
            else:
    ...
    def clear(self) -> None:
        ...
        self.device._channels[0].select_channel()
    ...
    def num_channels(self) -> int:
        """Returns the number of channels of a certain channel tuple"""
        return len(self._channels)

    # TODO: ? - Jeder Channel hat genau einen Marker oder?
    @property
    ...
    # TODO: In Channel Touple aendern
    @with_select
...
class AWGChannelTabor(BaseAWGChannel):
    ...
    def __init__(self, name: str, idn: int, tabor_device: AWGDeviceTabor, marker=False):
        ...
        self.add_feature(
            ChannelAmplitudeOffsetFeature(self._get_offset, self._set_offset, self._get_amplitude, self._set_amplitude))

        self.name = name
        self.marker = marker

        self._device = tabor_device
        self._channel_tuple: Optional[AWGChannelTupleTabor] = None

        # TODO: Amplitude und Offset Feature

    def select_channel(self) -> None:
        self.send_cmd(':INST:SEL {channel}'.format(channel=self.idn))

    def get_sample_rate(self) -> int:
        return int(float(self.send_query(':INST:SEL {channel}; :FREQ:RAST?'.format(channel=self.idn))))

    def _get_offset(self) -> float:
        return float(self.send_query(':INST:SEL {channel}; :VOLT:OFFS?'.format(channel=self.idn)))
    ...
    def _get_amplitude(self) -> float:
        coupling = self.send_query(':INST:SEL {channel}; :OUTP:COUP?'.format(channel=self.idn))
        if coupling == 'DC':
            return float(self.send_query(':VOLT?'))
        elif coupling == 'HV':
            return float(self.send_query(':VOLT:HV?'))
...
class TaborUndefinedState(TaborException):
    ...
    # TODO: so okay? - evtl noch anpassen - isinstance auche fuer channel?
    def __init__(self, *args, device: FeatureAble):
        super().__init__(*args)
        self.device = device
