...
class TaborChannelTuple(AWGChannelTuple):
    ...
    def read_waveforms(self) -> List[np.ndarray]:
        ...
        for segment in uploaded_waveform_indices:
            ...
            waveforms.append(device.read_act_seg_dat())  # ergebnis schon anders
        device.send_cmd(":TRAC:SEL {}".format(old_segment))
        # TODO: Fehler - anderes Ergebnis als der alte Treiber
        return waveforms
    ...
    def read_sequence_tables(self) -> List[Tuple[np.ndarray, np.ndarray, np.ndarray]]:
        ...
        for sequence in uploaded_sequence_indices:
            device.send_cmd(":SEQ:SEL {}".format(sequence))  # kommandos an das Geraet sind gleich wie im alten Treiber
            sequences.append(device.read_sequencer_table())  # sequences sind einzelnd noch gleich wie im alten Treiber
        device.send_cmd(":SEQ:SEL {}".format(old_sequence))
        return sequences  # sequences sind in beiden Treibern gleich
    ...
    def _amend_segments(self, segments: List[TaborSegment]) -> np.ndarray:
        ...
        # alle Variabel bis hier gleich wie im alten Treiber
